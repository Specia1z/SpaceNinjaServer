...
管理员还可以在 WebUI 的“账号倍率”页面按账号 ID 管理资源、Mod、信用点、专精经验、集团声望、午夜电波声望、遗物物品数量、遗物白金、任务白金和每日登录奖励倍率。页面支持搜索账号、启用/停用档案、重置和删除；旧版按显示名配置的资源/Mod 倍率会显示在相应账号中，保存新档案时自动迁移到账号 ID。倍率仅影响各字段对应的服务端结算路径，不修改客户端本地的掉落几率或经验计算。
