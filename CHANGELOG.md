...
- Settlement anti-cheat checks, configured under `antiCheat`. Only fields that reach the server through the mission settlement upload can be judged, so the checks cover the reward seed the client reports back, mission and alive time, the completions a single report claims, and equipment XP relative to mission length. Every hit writes a `[anti-cheat]` warning naming the account and the offending values; detection is on by default and changes nothing else.
