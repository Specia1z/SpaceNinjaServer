...
3. Move the remaining admin pages and inventory/clan workflows out of `static/webui/script.js`, with route-local state, API requests, and event handlers. Separate their markup from `static/webui/index.html` only after the existing single-page routing and localization behavior is covered by browser tests.
