...
const compatibleDescentTypes = new Set([...gameModes.map(gameMode => gameMode.type), "DT_PROTOFRAME"]);
const compatibleDescentChallenges = new Set([
    ...Object.values(challenges).flatMap(category => category.map(challenge => challenge.tag)),
    "Wisp",
    "Harrow",
    "Devil"
]);
const compatibleDescentLevels = new Set([
    ...gameModes.flatMap(gameMode => gameMode.levels),
    ...Object.values(challenges).flatMap(category => category.flatMap(challenge => challenge.forcedLevel ?? [])),
    "/Lotus/Levels/DevilTower/ProtoframeRoomWisp.level",
    "/Lotus/Levels/DevilTower/ProtoframeRoomHarrow.level",
    "/Lotus/Levels/DevilTower/BossArenaUriel.level"
]);
const compatibleDescentSpecs = new Set([
    ...secondarySpecs,
    ...Object.values(challenges).flatMap(category => category.flatMap(challenge => challenge.enemySpecs))
]);
const compatibleDescentAuras = new Set(
    Object.values(challenges).flatMap(category => category.flatMap(challenge => challenge.auras))
);
const post42DescentChallenges = new Set([
    "ToxicFire",
    "RocketsOnly",
    "HardShell",
    "FieryTrailRollers",
...
const post42DescentSpecs = new Set([
    "/Lotus/Types/Game/EnemySpecs/Tau/CoHCorpusGasExterminateMixed",
...
const post42DescentAuras = new Set([
    "/Lotus/Types/Scripts/Tau/CoH/Complications/ToxicFireEnhancementAura",
    "/Lotus/Types/Scripts/Tau/CoH/Complications/RocketSpawnAura",
    "/Lotus/Types/Scripts/Tau/CoH/Complications/RocketsOnlyAura",
    "/Lotus/Types/Scripts/Tau/CoH/Complications/HardShellEnhancementAura",
    "/Lotus/Types/Scripts/Tau/CoH/Complications/HeavyWeaponSpawnAura",
    "/Lotus/Types/Scripts/Tau/CoH/Complications/HeavyWeaponsOnlyAura",
    "/Lotus/Types/Scripts/Tau/CoH/Complications/SunlightAura",
...
const generateDescentFloor = (
    rng: SRng,
    forcedTypes: Record<number, string>,
    index: number,
    restrictTo42: boolean
): IDescentFloor => {
    ...
    for (const challengeCategory of gameMode.availableChallengeCategories) {
        for (const challenge of challenges[challengeCategory]) {
            if (!restrictTo42 || !post42DescentChallenges.has(challenge.tag)) {
    ...
    return {
        ...
        Specs: challenge.useSecondarySpec
            ? [
                  rng.randomElement(
                      restrictTo42 ? secondarySpecs.filter(spec => !post42DescentSpecs.has(spec)) : secondarySpecs
                  )!
              ]
            : challenge.enemySpecs,
        Auras: challenge.auras
    };
};

export const getDescent = (week: number, buildVersion: number = Number.MAX_SAFE_INTEGER): IDescent => {
    ...
    return {
        ...
        Challenges: [
            generateDescentFloor(rng, forcedTypes, 1, restrictTo42),
            generateDescentFloor(rng, forcedTypes, 2, restrictTo42),
            generateDescentFloor(rng, forcedTypes, 3, restrictTo42),
            generateDescentFloor(rng, forcedTypes, 4, restrictTo42),
            generateDescentFloor(rng, forcedTypes, 5, restrictTo42),
            generateDescentFloor(rng, forcedTypes, 6, restrictTo42),
            {
                Index: 7,
                Type: "DT_PROTOFRAME",
                Challenge: "Wisp",
            ...
            generateDescentFloor(rng, forcedTypes, 8, restrictTo42),
            generateDescentFloor(rng, forcedTypes, 9, restrictTo42),
            generateDescentFloor(rng, forcedTypes, 10, restrictTo42),
            generateDescentFloor(rng, forcedTypes, 11, restrictTo42),
            generateDescentFloor(rng, forcedTypes, 12, restrictTo42),
            generateDescentFloor(rng, forcedTypes, 13, restrictTo42),
            {
                Index: 14,
                Type: "DT_PROTOFRAME",
                Challenge: "Harrow",
            ...
            generateDescentFloor(rng, forcedTypes, 15, restrictTo42),
            generateDescentFloor(rng, forcedTypes, 16, restrictTo42),
            generateDescentFloor(rng, forcedTypes, 17, restrictTo42),
            generateDescentFloor(rng, forcedTypes, 18, restrictTo42),
            generateDescentFloor(rng, forcedTypes, 19, restrictTo42),
            generateDescentFloor(rng, forcedTypes, 20, restrictTo42),
            {
                Index: 21,
                Type: "DT_PROTOFRAME",
                Challenge: "Devil",
