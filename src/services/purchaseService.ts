...
export const handleDailyDealPurchase = async (
    ...
    const liveDailyDeal = getLiveDailyDealForPurchase(purchaseParams.StoreItem, buildLabel);
    const localDailyDeal = liveDailyDeal ? undefined : await DailyDeal.findOne({ StoreItem: purchaseParams.StoreItem });
    const dailyDeal = liveDailyDeal ?? localDailyDeal;
    if (!dailyDeal) {
        throw new Error("invalid or expired daily deal");
    }
    if (localDailyDeal) {
        localDailyDeal.AmountSold += 1;
        await localDailyDeal.save();
    }
